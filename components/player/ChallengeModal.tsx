...
export default function ChallengeModal({ location, ownership, myPlayer, myPos, onClose, onSuccess, onError }: Props) {
  ...
  async function upgradeDefense() {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { onError(`❌ ${data.error}`); return }
      onSuccess(`🛡️ Verdediging niveau ${data.defense_level}!`)
    } finally {
      setUpgrading(false)
    }
  }
  ...
  return (
    <div className="absolute inset-0 z-[2000] flex items-end" onClick={onClose}>
      <div className="w-full bg-[#1a1a2e] border-t border-white/10 rounded-t-2xl p-5 slide-up max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-white/50">{config.label} · +{location.crown_value} 👑/tick</p>
        ...
        {ownership && (
          <div className="mb-4 p-3 rounded-xl text-sm" style={{ background: `${ownership.player?.color}15`, border: `1px solid ${ownership.player?.color}30` }}>
            {isMine
              ? <p className="font-semibold text-emerald-400">✅ Jouw locatie {defLevel > 0 ? `· 🛡️ niveau ${defLevel}` : ''}</p>
              : <p><span style={{ color: ownership.player?.color }}>●</span> <span className="font-semibold">{ownership.player?.name}</span> bezit dit{defLevel > 0 ? <span className="text-orange-400"> · 🛡️ lvl {defLevel}</span> : null}</p>
            }
          </div>
        )}

        {/* Distance indicator */}
        <div className={`flex items-center gap-2 text-sm mb-5 ${inRange ? 'text-emerald-400' : 'text-orange-400'}`}>
          <span>{inRange ? '✅' : '📍'}</span>
          <span>{inRange ? 'Binnen bereik!' : `${distance}m weg — kom binnen ${location.claim_radius}m`}</span>
        </div>
        ...
        {isMine ? (
          <div className="space-y-3">
            {defLevel < 3 ? (
              <button onClick={upgradeDefense} disabled={upgrading || !canUpgrade} className="w-full py-3 bg-orange-600/80 hover:bg-orange-600 disabled:opacity-40 rounded-xl font-semibold text-sm transition-colors flex items-center justify-center gap-2">
                🛡️ Versterken naar niveau {defLevel + 1}
                <span className="text-xs opacity-70">({nextUpgradeCost} 👑{!canUpgrade && myPlayer.crowns < (nextUpgradeCost ?? 0) ? ` — je hebt ${myPlayer.crowns}` : ''})</span>
              </button>
            ) : (
              <div className="p-3 bg-orange-500/10 border border-orange-500/20 rounded-xl text-center text-sm text-orange-300 font-semibold">🛡️ Maximale verdediging (niveau 3)</div>
            )}
            <button onClick={onClose} className="w-full py-2 text-white/30 text-sm">Sluiten</button>
          </div>
        ) : phase === 'info' ? (
          <button onClick={() => setPhase('challenge')} disabled={!inRange} className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed rounded-xl font-bold transition-colors">
          ...
          <div className="space-y-3">
            ...
            {(location.challenge_type === 'quiz' || location.challenge_type === 'puzzle') && (
              <div className="space-y-3">
                ...
                <input value={answer} onChange={e => setAnswer(e.target.value)} placeholder="Jouw antwoord..."
            ...
            {location.challenge_type === 'photo' && (
              <div className="p-4 bg-white/5 rounded-xl">
                ...
                <p className="text-xs text-white/30 mt-2">(De begeleider beoordeelt de foto)</p>
              </div>
            )}
            {location.challenge_type === 'timed' && (
              <div className="p-4 bg-white/5 rounded-xl text-center">
                <p className="text-2xl mb-2">⏱️</p>
                <p className="font-semibold">Blijf 30 seconden op locatie</p>
              </div>
            )}
            <button onClick={submitClaim} disabled={loading || (location.challenge_type === 'quiz' && !answer)}
